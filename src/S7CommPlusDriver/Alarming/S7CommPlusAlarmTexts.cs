...
{
    public class S7CommPlusAlarmTexts
    {
        ...
        internal static S7CommPlusAlarmTexts FromNotificationBlob(ValueBlobSparseArray blob, int languageId)
        {
            var languages = FromNotificationBlobAllLanguages(blob);
            if (languages.TryGetValue(languageId, out var texts))
            {
                return texts;
            }
        ...
        internal static Dictionary<int, S7CommPlusAlarmTexts> FromNotificationBlobAllLanguages(ValueBlobSparseArray blob)
        {
            string s;
            int lcid;
            int textid;
            var textsByLanguage = new Dictionary<int, S7CommPlusAlarmTexts>();
            foreach (var v in blob.Value)
            {
                ...
                if (!textsByLanguage.TryGetValue(lcid, out var at))
                {
                    at = new S7CommPlusAlarmTexts { LanguageId = lcid };
                    textsByLanguage.Add(lcid, at);
                }
        ...
        {
            string s = "<S7CommPlusAlarmTexts LanguageId=\"" +  LanguageId.ToString() + "\">" + Environment.NewLine;
            ...
            s += "</S7CommPlusAlarmTexts>" + Environment.NewLine;
            return s;
        }

        internal static S7CommPlusAlarmTexts FirstOrEmpty(Dictionary<int, S7CommPlusAlarmTexts> textsByLanguage)
        {
            if (textsByLanguage == null || textsByLanguage.Count == 0)
            {
                return new S7CommPlusAlarmTexts();
            }

            return textsByLanguage.OrderBy(pair => pair.Key).First().Value;
        }
    }
}
